...
class ParallelTransactionsTest(TestCase):
    ...
    def create_same_accounts_subtest(self, iterations: int, expected_length: int):
        deposit_neon(self.evm_loader, self.operator_keypair,
                     self.user_account.eth_address, ONE_TOKEN)
        deposit_neon(self.evm_loader, self.operator_keypair,
                     self.second_account.eth_address, ONE_TOKEN)

        contract = create_contract_address(self.user_account, self.evm_loader)
        holder_acc = create_holder(self.operator_keypair)
        deployment_tx = make_deployment_transaction(
            self.user_account, BIG_CONTRACT_FILENAME)
        write_transaction_to_holder_account(
            deployment_tx, holder_acc, self.operator_keypair)
        ...
        for i in range(iterations):
            ...
            assert not ParallelTransactionsTest.check_iteration_deployed(
                deployment_receipt)
        ...
        try:
            ...
            ParallelTransactionsTest.check_account_initialized_in_another_trx_exception(
                e, contract.solana_address)
        ...
        data = get_account_data(
            solana_client, contract.solana_address, expected_length)
    ...
    def check_iteration_deployed(receipt: Any) -> bool:
        if receipt.value.transaction.meta.err:
            raise AssertionError(
                f"Can't deploy contract: {receipt.value.transaction.meta.err}")
