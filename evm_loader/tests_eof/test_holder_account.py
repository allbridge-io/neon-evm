...
def test_create_the_same_holder_account_by_another_user(operator_keypair, session_user):
    ...
    trx.add(
        ...
        solana_utils.create_holder_account(
            storage, session_user.solana_account.public_key)
    )

    with pytest.raises(solana.rpc.core.RPCException, match='already initialized'):
        solana_utils.send_transaction(
            solana_client, trx, session_user.solana_account)
...
def test_write_tx_to_holder_by_no_owner(operator_keypair, session_user, second_session_user, evm_loader):
    ...
    with pytest.raises(solana.rpc.core.RPCException, match="invalid owner"):
        write_transaction_to_holder_account(
            signed_tx, holder_acc, session_user.solana_account)
...
def test_delete_holder_by_no_owner(operator_keypair, user_account):
    holder_acc = create_holder(operator_keypair)
    with pytest.raises(solana.rpc.core.RPCException, match="invalid owner"):
        delete_holder(holder_acc, user_account.solana_account,
...
def test_write_to_not_finalized_holder(rw_lock_contract, user_account, evm_loader, operator_keypair, treasury_pool,
    ...
    account_data = solana_client.get_account_info(
        new_holder_acc, commitment=Confirmed).value.data
...
def test_holder_write_overflow(operator_keypair, treasury_pool, evm_loader,
    ...
    trx = Transaction()
    trx.add(make_WriteHolder(operator_keypair.public_key,
            holder_acc, b"\x00" * 32, overflow_offset, b"\x00" * 1))
