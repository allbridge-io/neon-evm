...
EVM_LOADER_SO = os.environ.get(
    "EVM_LOADER_SO", 'target/bpfel-unknown-unknown/release/evm_loader.so')
...
class SplToken:
    ...
    def mint(self, mint_id, recipient, amount, owner=None):
        if owner is None:
            ...
            self.call("mint {} {} {} --owner {}".format(mint_id,
    ...
    def create_token_account(self, token, owner=None):
        if owner is None:
            ...
            res = self.call(
                "create-account {} --owner {}".format(token, owner))
...
class solana_cli:
    ...
    def call(self, arguments):
        if self.acc is None:
            cmd = '{} --url {} {}'.format(path_to_solana,
                                          SOLANA_URL, arguments)
        else:
            cmd = '{} --keypair {} --url {} {}'.format(
                path_to_solana, self.acc.get_path(), SOLANA_URL, arguments)
        try:
            return subprocess.check_output(cmd, shell=True, universal_newlines=True)
        except subprocess.CalledProcessError as err:
...
class neon_cli:
    ...
    def call(self, arguments):
        cmd = 'neon-cli {} --commitment=processed --url {} {} -vvv'.format(
            self.verbose_flags, SOLANA_URL, arguments)
        proc_result = subprocess.run(
            cmd, shell=True, text=True, stdout=subprocess.PIPE, universal_newlines=True)
    ...
    def emulate(self, loader_id, sender, contract, data):
        ...
        proc_result = subprocess.run(
            cmd, input=data_json, text=True, stdout=subprocess.PIPE, universal_newlines=True)
    ...
    def call_contract_get_function(self, evm_loader, sender, contract, function_signature: str, constructor_args=None):
        ...
        result = self.emulate(evm_loader.loader_id, sender.eth_address.hex(
        ), contract.eth_address.hex(), data.hex())
...
class RandomAccount:
    ...
    def generate_key(self):
        cmd_generate = 'solana-keygen new --no-passphrase --outfile {}'.format(
            self.path)
        try:
...
class EvmLoader:
    def __init__(self, acc: OperatorAccount, program_id=EVM_LOADER):
        if program_id is None:
            ...
            result = json.loads(solana_cli(acc).call(
                'deploy {}'.format(EVM_LOADER_SO)))
    ...
    def deploy(self, contract_path, config=None):
        ...
        if config is None:
            output = neon_cli().call(
                "deploy --evm_loader {} {}".format(self.loader_id, contract_path))
        else:
    ...
    def ether2seed(self, ether: Union[str, bytes]):
        seed = b58encode(ACCOUNT_SEED_VERSION +
                         self.ether2bytes(ether)).decode('utf8')
        acc = account_with_seed(
            self.acc.get_acc().public_key, seed, self.loader_id)
        print('ether2program: {} {} => {}'.format(
            self.ether2hex(ether), 255, acc))
        return acc, 255

    def ether2program(self, ether: Union[str, bytes]) -> Tuple[str, int]:
        items = PublicKey.find_program_address(
            [ACCOUNT_SEED_VERSION, self.ether2bytes(ether)], PublicKey(EVM_LOADER))
    ...
    def deploy_checked(self, location, caller, caller_ether):
        ...
        ether = keccak_256(rlp.encode(
            (caller_ether, trx_count))).digest()[-20:]
    ...
    def create_ether_account_trx(self, ether: Union[str, bytes]) -> Tuple[Transaction, str]:
        ...
        data = bytes.fromhex(
            '28') + CREATE_ACCOUNT_LAYOUT.build(dict(ether=self.ether2bytes(ether)))
        trx = Transaction()
        trx.add(TransactionInstruction(
            ...
            keys=[
                ...
                AccountMeta(pubkey=PublicKey(SYSTEM_ADDRESS),
                            is_signer=False, is_writable=False),
                AccountMeta(pubkey=PublicKey(sol),
...
def get_account_data(solana_client: Client, account: Union[str, PublicKey, Keypair], expected_length: int) -> bytes:
    ...
    if len(info.data) < expected_length:
        print("len(data)({}) < expected_length({})".format(
            len(info.data), expected_length))
        raise Exception(
            "Wrong data length for account data {}".format(account))
    return info.data


def get_transaction_count(solana_client: Client, sol_account: Union[str, PublicKey]) -> int:
    info = get_account_data(solana_client, sol_account,
                            ACCOUNT_INFO_LAYOUT.sizeof())
...
def send_transaction(client: Client, trx, acc, wait_status=Confirmed):
    print("Send trx")
    result = client.send_transaction(trx, acc, opts=TxOpts(
        skip_confirmation=True, preflight_commitment=wait_status))
    tx = result.value
...
def deposit_neon(evm_loader: EvmLoader, operator_keypair: Keypair, ether_address: Union[str, bytes], amount: int):
    ...
    evm_token_authority, _auth_bump_seed = \
        PublicKey.find_program_address(
            [bytes("Deposit", encoding='utf-8')], evm_loader.loader_id)
    evm_pool_key = get_associated_token_address(
        evm_token_authority, NEON_TOKEN_MINT_ID)

    signer_token_pubkey = get_associated_token_address(
        operator_keypair.public_key, NEON_TOKEN_MINT_ID)
    trx = Transaction()
    trx.add(
...
def write_transaction_to_holder_account(
    ...
    while len(rest):
        ...
        trx = Transaction()
        trx.add(make_WriteHolder(operator.public_key,
                holder_account, signed_tx.hash, offset, part))
        receipts.append(
...
def execute_transaction_steps_from_account(operator: Keypair, evm_loader, treasury, storage_account,
    ...
    send_transaction_step_from_account(
        operator, evm_loader, treasury, storage_account, additional_accounts, 1, signer)
    if steps_count > 0:
        steps_left = steps_count
        while steps_left > 0:
